...
```{r}
library(heplots)
boxM(ex_5_full[, -3], ex_5_full[, 3])
```
We have to hypothesis: <br>
- h0: The covariance matrices are equal  
- h1: at least two of the covariance matrices are not equal <br>
We can then reference a chi_squared table. The value needs to be above the specified chi-squared value for significance 0.05. If the value is above we accept h0 otherwise we reject h0: <br> 

![Chi squared table](chi_test.png)  
